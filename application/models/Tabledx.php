...
class Tabledx extends CI_Model{
    ...
    public function order($key = "", $var = "ASC")
    {
        $this->orderby = " ORDER BY $key $var ";
    }
    ...
    public function getResult($condtype = 0)
    {
        if(count($this->condition) > 0){
            ...
            $this->debugsql = "SELECT $this->selectparams FROM $this->tb $this->leftjoin $condconf AND $this->tb.$this->delete_time = '0' $this->orderby $this->lmt ";
            if ($this->debuger == true) {
              echo "<pre>";
              echo $this->debugsql;
              echo "</pre>";
            }
            return $this->db->query("SELECT $this->selectparams FROM $this->tb $this->leftjoin $condconf AND $this->tb.$this->delete_time = '0' $this->orderby $this->lmt ")->result();
        }else{
            $this->debugsql = "SELECT $this->selectparams FROM $this->tb $this->leftjoin WHERE $this->tb.$this->delete_time = '0' $this->orderby $this->lmt ";
            if ($this->debuger == true) {
              echo "<pre>";
              echo $this->debugsql;
              echo "</pre>";
            }
            return $this->db->query("SELECT $this->selectparams FROM $this->tb $this->leftjoin WHERE $this->tb.$this->delete_time = '0' $this->orderby $this->lmt ")->result();
        }
    }
    ...
    public function addUpdated($date = 'Y-m-d h:i:s')
    {
        $getId = post('id');
        if ($getId != "") {
          $this->databackup = $this->db->query("SELECT * FROM $this->tb WHERE $this->key = '$getId' ")->row();
        }
    ...
    public function row($dataName = null, $err = null, $condtype = 0)
    {
        ...
        if(count($this->condition) > 0){
            ...
            $this->debugsql = "SELECT $this->selectparams FROM $this->tb $this->leftjoin $condconf $savemoment $this->orderby $this->lmt";
            if ($this->debuger == true) {
              echo "<pre>";
              echo $this->debugsql;
              echo "</pre>";
            ...
            $elm = $this->db->query("SELECT $this->selectparams FROM $this->tb $this->leftjoin $condconf $savemoment $this->orderby $this->lmt")->row();
            if ($elm != null) {
                if ($dataName != null) {
    ...
    public function updateData($var = ["set" => [], "condition" => []])
    {
        ...
        if ($this->databackup != "") {
            ...
            $cek = $this->db->query("SELECT
              ...
              FROM
          	information_schema.TABLES WHERE table_schema = '$dbname' AND table_name = 'log'
            ")->row();

            if ($cek == NULL) {
              $createLog = $this->db->query("CREATE TABLE log (
...
");
              if ($createLog) {
                ...
                $tt = date('Y-m-d h:i:s');
                $ts = datalogin('nama');
                $td = strigToBinary(json_encode($this->databackup, true));
                $tid = $this->databackup->id;

                $this->db->query("INSERT INTO log
                  (tanggal, username, table_name, data_id, data_update) VALUES
                  ('$tt', '$ts', '$this->tb', '$tid', \"$td\" ) ");
